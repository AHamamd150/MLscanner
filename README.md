...
## Introduction
&emsp;  Machine learning (ML) techniques are natural contenders for alleviating such difficulties for their ability to find concealed patterns in large and complex data sets. Recently, it was proposed a general purpose method known as ML Scan (MLS), where the authors use a deep neural network to iteratively probe the parameter space, starting with points randomly distributed, where the sampling is improved incrementally with active learning. Conversely, the active learning methods are based on finding decision boundaries of the allowed subspace. 
